...
*Setup instructions for a gitian build of Volt using a Debian VM or physical system.*
...
Clone the git repositories for volt and gitian and then checkout the volt version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/volt-crypto/volt.git
cd volt
...
Follow the instructions in [doc/release-process.md](release-process.md) in the volt repository
...
To build Volt (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the volt repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/volt/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/volt-crypto/volt
...
For example:
```bash
URL=https://github.com/crowning-/volt.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit volt=${COMMIT} --url volt=${URL} ../volt/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit volt=${COMMIT} --url volt=${URL} ../volt/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit volt=${COMMIT} --url volt=${URL} ../volt/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/volt-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/volt-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/volt-build.assert
```
...
[volt/gitian.sigs](https://github.com/volt-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.

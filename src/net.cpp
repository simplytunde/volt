...
static CSemavolt* semOutbound = NULL;
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        string strDesc = "Volt " + FormatFullVersion();
...
void static ProcessOneShot()
{
    ...
    CAddress addr;
    CSemavoltGrant grant(*semOutbound, true);
    if (grant) {
...
void ThreadOpenConnections()
{
    ...
    while (true) {
        ...
        CSemavoltGrant grant(*semOutbound);
        boost::this_thread::interruption_point();
...
bool OpenNetworkConnection(const CAddress& addrConnect, CSemavoltGrant* grantOutbound, const char* pszDest, bool fOneShot)
{
    //
...
bool BindListenPort(const CService& addrBind, string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Volt Core is probably already running."), addrBind.ToString());
        else
...
void StartNode(boost::thread_group& threadGroup)
{
    ...
    if (semOutbound == NULL) {
        ...
        semOutbound = new CSemavolt(nMaxOutbound);
    }

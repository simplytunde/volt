...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeVoltAmount"))
        settings.setValue("nAnonymizeVoltAmount", 1000);

    nAnonymizeVoltAmount = settings.value("nAnonymizeVoltAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeVoltAmount"))
        SoftSetArg("-anonymizevoltamount", settings.value("nAnonymizeVoltAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVoltAmount:
            return QVariant(nAnonymizeVoltAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeVoltAmount:
            nAnonymizeVoltAmount = value.toInt();
            settings.setValue("nAnonymizeVoltAmount", nAnonymizeVoltAmount);
            emit anonymizeVoltAmountChanged(nAnonymizeVoltAmount);
            break;
        case CoinControlFeatures:
